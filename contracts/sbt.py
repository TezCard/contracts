...
#         self.data.t_joins[user] = sp.none
    ...
    def list_participant(self, params):
        ...
        sp.if t_list_rank_params.type == 1:
            ...
            sp.for x in self.data.t_rank_joins_map.keys():
    ...
    def mint(self, param):
        ...
        tag = sp.bool(False)
        sp.for x in self.data.t_rank_joins_map.keys():
           sp.if self.data.t_rank_joins_map[x].win_users.contains(sp.sender):
                tag = sp.bool(True)
    ...
    def get_balance_of(self):
        ...
        sp.if self.data.owner_address_map.keys().contains(sp.sender):
            sp.result(1)
        sp.else:
            sp.result(0)
